...
-- Champion progression, win-streak, shiny ownership, and quest claims
-- (reconciliation block — also applied via supabase/migrations for the CLI path).
...
create or replace view user_stats as
  select
    ...
    coalesce(played.cnt, 0) - coalesce(won.cnt, 0) as losses,
    case when coalesce(played.cnt, 0) = 0 then 0.0
         else round(100.0 * coalesce(won.cnt, 0) / played.cnt, 1) end as win_pct,
    coalesce(owned.cnt, 0) as cards_owned,
  ...
  from users u
  left join (select user_id, count(*)::int as cnt from (
      select p1_user_id as user_id from matches where p1_user_id is not null
      union all select p2_user_id as user_id from matches where p2_user_id is not null
